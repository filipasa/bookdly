...
function setupCompleted( status, view )
{
    if ( status )
    {
        $('div[data-step="stripe_connect"]').replaceWith( booknetic.htmlspecialchars_decode( view ) );
    }
}
...
(function ($)
{
    ...
    {
        $('#booknetic_settings_area').on('click', '.stripe_connect_verify_btn', function()
        {
            ...
            booknetic.ajax('stripe_connect_settings.generate_verify_link', new FormData, function(result)
            {
