...
class TwilioWhatsappWorkflowDriver extends WorkflowDriver
{

    protected $driver = 'twilio-whatsapp';

    public function __construct()
    {
        $this->setName( bkntc__('Send WhatsApp message via Twilio') );
	    $this->setEditAction( 'twilio_whatsapp_workflow', 'workflow_action_edit_view' );
    }
    ...
    public function send( $sendTo, $body, $actionSettings)
    {
	    ...
	    if( Capabilities::getLimit( 'twilio_whatsapp_allowed_max_number' ) <= $logCount && Capabilities::getLimit( 'twilio_whatsapp_allowed_max_number' ) > -1 )
	    {
		    return false;
	    }

	    $whatsapp_account_sid		= Helper::getOption('whatsapp_account_sid', '', false);
	    $whatsapp_auth_token		= Helper::getOption('whatsapp_auth_token', '', false);
