(function ($)
{
	...
	$(document).ready(function()
	{
		...
		{
			sendTwilioWhatsapp(function ()
			{
				booknetic.modal('<div class="p-3 pt-5 pb-5">' +
					'<div class="mb-2">' +
					'<input class="form-control" id="send_test_whatsapp_to" placeholder="'+booknetic.__('To')+'">' +
					'</div>' +
				...
				$('#send_test_btn').click(function ()
				{
					...
					booknetic.ajax( 'twilio_whatsapp_workflow.workflow_action_send_test_data', { id: workflow_action_id, to: $('#send_test_whatsapp_to').val()}, function ()
					{
		...
		function sendTwilioWhatsapp( callback )
		{
			var to	    = $("#input_to").val(),
				body    = $("#input_body").val(),
			...
			booknetic.ajax('twilio_whatsapp_workflow.workflow_action_save_data', data, function()
			{
		...
		booknetic.initKeywordsInput(
			$('#input_body'),
			workflow_twilio_whatsapp_action_all_shortcodes_obj
		);
