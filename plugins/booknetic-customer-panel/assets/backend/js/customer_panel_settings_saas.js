(function ($)
{
    ...
    $(document).ready(function()
    {
        ...
        $('#booknetic_settings_area').off('click', '.settings-save-btn');
        ...
        bookneticSettings.setOnSave(function ()
        {
            var customer_panel_page_id					            = $("#input_customer_panel_page_id").select2('val'),
                ...
                time_restriction_to_make_changes_on_appointments 	= $("#input_time_restriction_to_make_changes_on_appointments").select2('val'),
            ...
            data.append('customer_panel_allow_cancel', customer_panel_allow_cancel);
            ...
            booknetic.ajax('customerpanel.save_customer_panel_settings_saas', data, function ()
            {
                booknetic.toast(booknetic.__('saved_successfully'), 'success');
            });
        });

        $('#booknetic_settings_area').on('change', 'input[name="input_customer_panel_enable"]', function()
        {
        ...
        $("#input_time_restriction_to_make_changes_on_appointments").select2({
            theme: 'bootstrap',
            placeholder: booknetic.__('select')
        });
