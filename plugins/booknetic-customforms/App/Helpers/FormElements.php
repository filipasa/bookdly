...
class FormElements
{
	...
	public static function formElement( $preId, $type, $label, $isRequired = false, $helptext = '', $value = '', $inputId = '', $options = '', $file_name ='', $isBackend = false )
	{
		...
		{
			$return = self::formFileMultiple( $label, $isRequired, $helptext, $placeholder, $value, $inputId, $file_name );
		}
		else if( $type == 'link' )
		{
	...
	public static function formFileMultiple( $label = '', $isRequired = false, $helptext = '', $placeholder = '', $value = '', $inputId = '', $file_name = '' )
	{
        $randId = md5(uniqid());
        ...
        if (is_array($files)) {
            foreach ($files as $file) {
                $filesHTML .= '<div class="custom-file-item" style="margin-top: 5px; display: inline-flex; align-items: center; gap: 6px;"><img src="' . Helper::assets('icons/unsuccess.svg') . '" class="remove_custom_file_btn" data-placeholder="' . htmlspecialchars($placeholder) . '" data-save-custom-data="' . (int)$inputId . '" data-file-path="'.htmlspecialchars($file['path']).'" style="cursor: pointer; width: 14px; height: 14px;"> <a href="' . Helper::uploadedFileURL( $file['path'], 'Customforms' ) . '" target="_blank">'.htmlspecialchars($file['name']) . '</a></div>';
            }
        }

		return '<label data-label="true" data-required="' . ( $isRequired ? 'true' : 'false' ) . '">' . strip_tags($label, '<a><b><i><u>') . '</label>
				<input id="'.$randId.'" placeholder="' . htmlspecialchars($placeholder) . '" type="file" multiple class="form-control" data-input-id="' . (int)$inputId .'" data-type="file_multiple">
				<label for="'.$randId.'"  class="form-control" data-label="'.bkntc__('BROWSE').'" data-has-label="true">' . htmlspecialchars($placeholder) . '</label>
                <div class="uploaded-files-container d-flex flex-column align-items-start">' . $filesHTML . '</div>
                <div class="staged-files-container d-flex flex-column align-items-start" style="margin-top: 5px;"></div>
				<span class="help-text">' . htmlspecialchars($helptext) . '</span>';
	}
