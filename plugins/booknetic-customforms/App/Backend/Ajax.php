        ...
        foreach ( $inputs AS $input )
		{
			if (
				! (
					...
					&& isset( $input['type'] ) && is_string($input['type']) && in_array( $input['type'], ['label', 'text', 'textarea', 'number', 'date', 'time', 'select', 'checkbox', 'radio', 'file', 'file_multiple', 'link', 'email', 'phone'] )
				)
			)
			{
				continue;
