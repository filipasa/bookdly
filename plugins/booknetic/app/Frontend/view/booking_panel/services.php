...
if (count($parameters['services']) == 0) {
    ...
    if ($parameters['show_service_search'] === 'on') {
        echo '<div class="booknetic_service_search_wrapper">';
        echo '<input type="text" class="booknetic_service_search_input" placeholder="' . esc_attr(bkntc__('Search...')) . '">';
        echo '</div>';
    }
    ...
    $isAccordionEnabled = $parameters['hide_accordion_default'];
    ...
    foreach ($services as $eq => $serviceInf) {
        if ($lastCategoryPrinted != $serviceInf['category_id']) {
            if ($lastCategoryPrinted !== null) {
                // Close previous service cards container and category wrapper
                echo '</div></div>';
            }

            // Determine if collapsed by default
            $collapsedClass = ($isAccordionEnabled === 'on') ? 'collapsed' : '';
            
            // Start new category wrapper
            echo '<div class="booknetic_service_category_wrapper ' . $collapsedClass . '">';
            echo '<div data-parent="'.$isFirstCategoryService .'" class="booknetic_service_category booknetic_fade ' . $collapsedClass . '">' . htmlspecialchars($serviceInf['category_name']) . '<span data-parent="'. $isFirstCategoryService .'"></span></div>';
            echo '<div class="booknetic_service_cards_container">';
    ...
    if ($lastCategoryPrinted !== null) {
        // Close last cards container and category wrapper
        echo '</div></div>';
    }
