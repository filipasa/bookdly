...
class Ajax extends FrontendAjax
{
    ...
    public function location()
    {
        ...
        $categoryNameSubQuery = LocationCategory::query()
            ->where('id', '=', DB::field('category_id', Location::getTableName()))
            ->select('name', true)
            ->limit(1);

        $locationsQuery = $appointmentObj->getAvailableLocations()
            ->select('*')
            ->selectSubQuery($categoryNameSubQuery, 'location_categories_name')
            ->withTranslations();

        if ($appointmentObj->locationCategoryId > 0) {
            $locationsQuery->where('category_id', $appointmentObj->locationCategoryId);
        }
        ...
        return $this->view('booking_panel.locations', [
            ...
            'hide_location_accordion_default' => Helper::getOption('hide_location_accordion_default', 'off'),
        ]);
    }
    ...
    public function get_booking_panel()
    {
        $atts = [
            ...
            'location_category' => Post::int('location_category'),
    ...
    public function service()
    {
        ...
        if (empty($services)) {
            return $this->view('booking_panel.services', [
                'services'              => $services,
                'show_service_search'   => Helper::getOption('show_service_search', 'off'),
                'hide_accordion_default' => Helper::getOption('hide_accordion_default', 'off'),
            ]);
        }
    ...
    public function information()
    {
        ...
        $emailConfirmationEnabled = Helper::getOption('require_email_confirmation', 'off') === 'on';
        $showEmailConfirmInput = $emailConfirmationEnabled && ! $hideEmailInput && ! $disableEmailInput;
        ...
        $parameters = [
            ...
            'show_email_confirm_input' => $showEmailConfirmInput,
    ...
    public function confirm()
    {
        ...
        $responseData['google_calendar_url'] = AjaxHelper::addToGoogleCalendarURL($firstAppointment, $ar->appointments);
        $responseData['icalendar_url'] = AjaxHelper::addToiCalendarURL($firstAppointment, $ar->appointments);
    ...
    public function check_customer_exist()
    {
        ...
        try {
            do_action('bkntc_validate_customer_booking', $appointmentObj);
        } catch (Exception $e) {
            return $this->response(false, $e->getMessage());
        }
