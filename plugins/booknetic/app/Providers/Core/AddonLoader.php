...
use BookneticApp\Providers\Router\BookneticParameterResolver;
...
use BookneticApp\Providers\Router\WordPress\WordPressRouteRegistrar;
...
class AddonLoader
{
    ...
    {
        $addonDir  = dirname((new \ReflectionClass(static::class))->getFileName());
        $cachePath = WP_PLUGIN_DIR . '/' . static::getAddonSlug() . '/cache/di_cache.php';

        if (file_exists($cachePath)) {
            ContainerLoader::loadFromCache(require $cachePath);
        } else {
            $scanner = new ServiceScanner($addonDir);
            ContainerLoader::loadFromScan($scanner->scan());
        }
    }

    public function initRestRoutes(): void
    {
        $addonDir  = dirname((new \ReflectionClass(static::class))->getFileName());
        $cachePath = WP_PLUGIN_DIR . '/' . static::getAddonSlug() . '/cache/routes.php';

        if (file_exists($cachePath)) {
            $cache  = new RouteCache($cachePath);
            $routes = RouteCollection::fromCache($cache);
        } else {
            $scanner = new RouteScanner($addonDir);
            $routes  = new RouteCollection($scanner->scan());
        }
        ...
        $registrar = new WordPressRouteRegistrar(
            ...
            new BookneticParameterResolver(),
        );
