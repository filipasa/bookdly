...
namespace BookneticApp\Providers\Core
{
    ...
    class Bootstrap
    {
        ...
        public function __construct()
        {
            ...
            RestRoute::init();
            $this->initAttributeRouter();

            add_action('plugins_loaded', static function () {
                static::$addons = apply_filters('bkntc_addons_load', []);

                foreach (static::$addons as $addon) {
                    if (!$addon->shouldBlock()) {
                        $addon->initDependencies();
                        $addon->initRestRoutes();
                    }
                }
            });
        ...
        private function initAttributeRouter(): void
        {
            $routeCachePath = dirname(__DIR__, 3) . '/cache/routes.php';

            if (file_exists($routeCachePath)) {
                $cache = new \BookneticApp\Providers\Router\RouteCache($routeCachePath);
                $routes = \BookneticApp\Providers\Router\RouteCollection::fromCache($cache);
            } else {
                $scanner = new \BookneticApp\Providers\Router\RouteScanner(__DIR__ . '/../../Backend');
                $routes = new \BookneticApp\Providers\Router\RouteCollection($scanner->scan());
            }

            foreach ($routes->controllers() as $controllerClass) {
                if (!Container::has($controllerClass)) {
                    Container::add($controllerClass);
                }
            }

            $registrar = new \BookneticApp\Providers\Router\WordPress\WordPressRouteRegistrar(
                'booknetic/v1',
                fn (string $class) => Container::get($class),
                new \BookneticApp\Providers\Router\BookneticParameterResolver(),
            );
