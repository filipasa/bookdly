...
class ShortCodeServiceImpl
{
    public static function replace($text, $data)
    {
        if (!empty($data['appointment_id'])) {
            ...
            $calTitle = $appointmentSO->getServiceInf()->name;
            $calStartsAt = $appointment->starts_at;
            $calEndsAt = $appointment->ends_at;

            if ($appointment->payment_id > 0) {
                $siblingAppts = Appointment::where('payment_id', $appointment->payment_id)->orderBy('starts_at', 'ASC')->fetchAll();
                if (!empty($siblingAppts)) {
                    $sNames = [];
                    foreach ($siblingAppts as $sAppt) {
                        $sModel = Service::get($sAppt->service_id);
                        if ($sModel) {
                            $sNames[] = $sModel->name;
                        }
                    }
                    if (!empty($sNames)) {
                        $calTitle = implode(', ', array_unique($sNames));
                    }
                    $calStartsAt = $siblingAppts[0]->starts_at;
                    $calEndsAt = $siblingAppts[count($siblingAppts) - 1]->ends_at;
                }
            }

            $addToGoogleCalendarURL = 'https://www.google.com/calendar/render?action=TEMPLATE&text='
                . urlencode($calTitle)
                . '&dates=' . (Date::UTCDateTime($calStartsAt, 'Ymd\THis\Z') . '/'
                    . Date::UTCDateTime($calEndsAt, 'Ymd\THis\Z'))
        ...
        if (strpos($text, '{all_staff_list}') !== false) {
            $staffs = Staff::query()->whereIsNotNull('user_id')->select(['user_id'])->fetchAll();
            $userIds = implode(',', array_map(static function ($staff) {
                return $staff->user_id;
            }, $staffs));

            $text = str_replace('{all_staff_list}', $userIds, $text);
        }
        ...
        if (!empty($data['location_id'])) {
            ...
            if (!empty($locationInf->category_id)) {
                $locationCategory = LocationCategory::query()
                    ->withTranslations()
                    ->get($locationInf->category_id);
                if ($locationCategory) {
                    $locationCategoryName = $locationCategory->name;
                }
            }
