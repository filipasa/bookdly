...
class Helper
{
    ...
    public static function profileImage($image, $module = 'Base', $is_addon = false)
    {
        ...
        if ($image === 'logo') {
            return self::assets('images/logo-black.svg', $module, $is_addon);
        }
    ...
    {
        if (
            ! isset(self::$translationsCache[ 'bkntc_waiting_for_wp_functions_to_load' ]) &&
            function_exists('wp_get_current_user') &&
            ! Permission::isBackEnd()
        ) {
            $translations = Translation::query()
                ->where('table_name', 'options')
                ->where('locale', Helper::getLocale())
                ->fetchAll();
            self::$translationsCache = Helper::assocByKey($translations, 'column_name');
    ...
    public static function getOption($optionName, $default = null, $multi_tenant_option = true)
    {
        ...
        if ($optionName === 'woocommerce_order_details') {
            $translation = Translation::query()
                ->where('column_name', 'woocommerce_order_details')
                ->where('locale', get_locale())
                ->fetch();
    ...
    {
        Container::get(TranslationService::class)->saveOptions($translations, $options);
