...
{
    public const MENU_TYPE_LEFT = 1;
    public const MENU_TYPE_TOP_RIGHT = 2;
    public const MENU_TYPE_TOP_LEFT = 3;
    public const MENU_TYPE_BOOSTORE = 4;
    ...
    {
        $items = ! empty(static::$items[ $type ]) ? static::$items[ $type ] : [];

        if ( \BookneticApp\Providers\Helpers\Helper::isSaaSVersion() && class_exists('\\BookneticApp\\Providers\\Core\\Permission') ) {
            $tenantId = \BookneticApp\Providers\Core\Permission::tenantId();
            if ( $tenantId > 0 && \BookneticApp\Providers\Core\Permission::isAdministrator() ) {
                $tenantInf = \BookneticApp\Providers\Core\Permission::tenantInf();
                if ($tenantInf) {
                    $expiresIn = $tenantInf->expires_in;
                    $isExpired = false;
                    if (class_exists('\\BookneticApp\\Providers\\Helpers\\Date')) {
                        $isExpired = \BookneticApp\Providers\Helpers\Date::epoch(\BookneticApp\Providers\Helpers\Date::dateSQL()) > \BookneticApp\Providers\Helpers\Date::epoch($expiresIn);
                    }
                    if ($isExpired && class_exists('\\BookneticSaaS\\Models\\Tenant')) {
                        $isExpired = !\BookneticSaaS\Models\Tenant::haveEnoughBalanceToPay();
                    }
                    
                    if ($isExpired) {
                        $filtered = [];
                        foreach ($items as $item) {
                            if ($item->getSlug() === 'billing') {
                                $filtered[] = $item;
                            }
                        }
                        $items = $filtered;
