...
abstract class BaseEventService
{
    ...
    protected function getCommonSelectionData(array $idsByType): array
    {
        ...
        if (!empty($idsByType['location_categories']) && is_array($idsByType['location_categories'])) {
            $locationCategories = LocationCategory::query()
                ->select(['id', 'name'])
                ->where('id', 'IN', $idsByType['location_categories'])
                ->fetchAllAsArray();

            if (!empty($locationCategories)) {
                $data['location_categories'] = $locationCategories;
            }
        }
