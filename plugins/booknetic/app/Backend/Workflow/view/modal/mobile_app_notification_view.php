...
?>
<script>
    let workflow_mobile_app_notification_action_all_shortcodes = <?php echo json_encode($parameters['all_shortcodes']) ?>;

    let workflow_mobile_app_notification_action_all_shortcodes_obj = {};
    workflow_mobile_app_notification_action_all_shortcodes.forEach((value,index)=>{
        workflow_mobile_app_notification_action_all_shortcodes_obj[value.code] = value.name;
    });
</script>
...
<script type="text/javascript" src="<?php echo Helper::assets('js/mobile_app_notification.js', 'Workflow')?>"></script>
        ...
        <form id="editWorkflowActionForm">
                ...
                <div class="form-group col-md-12">
                    ...
                    <select id="input_to" class="form-control" multiple>
                        <?php foreach ($parameters['users'] as $user) :?>
                            <option value="<?php echo $user['ID']?>" <?php echo in_array($user['ID'], $parameters['to']) ? 'selected' : ''; ?> ><?php echo htmlspecialchars($user['user_login']); ?></option>
                        <?php endforeach;?>
                    </select>
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group col-md-12">
                    ...
                    <input type="text" id="input_title" class="form-control" value="<?php echo htmlspecialchars($parameters["title"] ?? '')?>">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group col-md-12">
                    ...
                    <textarea class="form-control required" id="input_description"><?php echo empty($parameters["description"]) ? '' : htmlspecialchars($parameters["description"]);?></textarea>
                </div>
            </div>
