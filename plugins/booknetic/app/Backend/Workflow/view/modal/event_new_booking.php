...
?>
        ...
        <div class="form-row">
            <div class="form-group col-md-12">
                <label for="input_location_categories"><?php echo bkntc__('Location category filter')?></label>
                <select class="form-control" id="input_location_categories" multiple>
                    <?php
                    foreach ($parameters['location_categories'] as $locationCategory) {
                        echo '<option value="' . (int)$locationCategory['id'] . '" selected>' . htmlspecialchars($locationCategory['name']) . '</option>';
                    }
?>
                </select>
            </div>
