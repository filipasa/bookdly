...
class ActionsAjax extends \BookneticApp\Providers\Core\Controller
{
    ...
    public function __construct(WorkflowEventsManager $workflowEventsManager)
    {
        ...
        $this->workflowActionRepository = Container::get(WorkflowActionRepository::class);
        $this->service = Container::get(WorkflowActionService::class);
    }
    ...
    private function getStaffUserOptions(bool $hasIdShortcodes): array
    {
        ...
        $users[] = [
            'ID' => '{all_staff_list}',
            'user_login' => bkntc__('All staff list'),
        ];
    ...
    public function mobile_app_notification_view()
    {
        $id = Post::int('id');

        $workflowActionInfo = $this->service->get($id);
        ...
        return $this->modalView('mobile_app_notification_view', [
            'action_info' => $workflowActionInfo,
            'users' => $this->getStaffUserOptions($hasIdShortcodes),
            'to' => $this->resolveSelectedStaffIds($data['to'] ?? null),
            'all_shortcodes' => $subjectAndBodyShortcodes,
    ...
    public function mobile_app_notification_save()
    {
        $id = Post::int('id');
        $to = Post::string('to');
        $title = Post::string('title');
        ...
        if (empty($to) || empty($title) || empty($description)) {
            return $this->response(false, bkntc__('Fields are invalid'));
        }
