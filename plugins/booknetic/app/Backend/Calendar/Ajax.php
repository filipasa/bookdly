...
class Ajax extends \BookneticApp\Providers\Core\Controller
{
    ...
    private function getCalendarPostData(): array
    {
        ...
        return  [
            ...
            'staffFilter'	    =>	$this->filterPositiveIntegerValues(Post::array('staff')),
            'locationFilter'	=>	$this->filterPositiveIntegerValues(Post::array('locations')),
            'servicesFilter'	=>	$this->filterPositiveIntegerValues(Post::array('services')),
            'statusesFilter'	=>	$this->filterStringValues(Post::array('statuses')),
            'paymentFilter'	    =>	$this->filterStringValues(Post::array('payments'))
        ];
    }
     ...
     * @return array
     */
    private function filterPositiveIntegerValues(array $postData): array
    {
        $filteredArray = [];
    ...
    private function filterStringValues(array $postData): array
    {
        $filteredArray = [];

        foreach ($postData as $dataId) {
            if (is_string($dataId)) {
                $filteredArray[] = $dataId;
            }
        }
