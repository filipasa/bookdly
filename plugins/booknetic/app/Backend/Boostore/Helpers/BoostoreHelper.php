...
class BoostoreHelper
{
    ...
    public static function getSubscription(): ?array
    {
        $apiClient = Container::get(FSCodeAPIClient::class);
        $product = Helper::isSaaSVersion() ? 'booknetic-saas' : 'booknetic';
        $endpoint = $product . '/product/subscription';

        $response = $apiClient->requestNew($endpoint);
        ...
        return $apiRes['subscription'] ?? null;
    }

    public static function generateSubscriptionUpgradeUrl(string $addon = ''): ?string
    {
        $apiClient = Container::get(FSCodeAPIClient::class);
        $product = Helper::isSaaSVersion() ? 'booknetic-saas' : 'booknetic';
        $endpoint = $product . '/product/subscription/generate-url';
    ...
    public static function checkAllAddonsUnowned(): bool
    {
        ...
        $total = $addons['total'] ?? count($addons['items']);

        return count($unownedAddons) === $total - 1;
    }
