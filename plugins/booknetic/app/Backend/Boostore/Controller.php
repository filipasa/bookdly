...
class Controller extends \BookneticApp\Providers\Core\Controller
{
    public function index()
    {
        $this->view('index', [
            ...
            'subscription' => BoostoreHelper::getSubscription(),
        ]);
    }
    ...
    public function details()
    {
        ...
        $addonData = BoostoreHelper::get('info/' . $addonSlug);

        if (empty($addonData) || ! isset($addonData['slug'])) {
        ...
        $cartItems = Cart::query()
            ...
            ->where('active', 1)
            ->fetchAll();
        $cartSlugs = array_column($cartItems, 'slug');

        $addonData['is_installed'] = ! empty(BoostoreHelper::getAddonSlug($addonData['slug']))
            && file_exists(realpath(WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . BoostoreHelper::getAddonSlug($addonData['slug'])));

        $addonData['in_cart'] = in_array($addonData['slug'], $cartSlugs, true) && ! $addonData['is_installed'];

        $addon = AddonDetailResponse::fromArray($addonData);
        $subscription = SubscriptionResponse::fromArray(BoostoreHelper::getSubscription());
    ...
    public function my_purchases(): void
    {
        ...
        $this->view('my_purchases', [
            ...
            'cart_items_count' => Cart::query()->where('active', 1)->count(),
            'subscription' => BoostoreHelper::getSubscription(),
        ]);
    }
