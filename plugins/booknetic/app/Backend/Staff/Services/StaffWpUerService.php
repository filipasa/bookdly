...
{
    /**
     * Validates that an email address is not associated with a disallowed WP role.
     *
     * @throws StaffValidationException
     */
    public function validateEmailForLogin(string $email): void
    {
     ...
     */
    public function handleDeletion(int $wpUserId, bool $deleteCompletely): void
    {
    ...
    {
        return array_map(static fn ($user) => new \BookneticApp\Backend\Staff\DTOs\Response\StaffWpUserSelectOptionResponse(
            $user->ID,
            $user->display_name,
            $user->user_email
        ), get_users([
