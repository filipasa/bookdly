...
class StaffRelationService
{
    ...
    {
        $existing = $this->serviceStaffRepository->getByStaffId($dto->id);

        $existingServiceIds = array_map(
            static fn (array $row) => (int)$row['service_id'],
            $existing
        );

        $incomingServiceIds = array_unique(
            array_map('intval', $dto->services)
        );

        $toDelete = array_diff($existingServiceIds, $incomingServiceIds);
        $toInsert = array_diff($incomingServiceIds, $existingServiceIds);

        foreach ($toDelete as $serviceId) {
            $this->serviceStaffRepository->delete(
                $dto->id,
                $serviceId
            );
        }
    ...
    public function deleteAllForStaff(int $staffId): void
    {
        ServiceStaff::query()->where('staff_id', $staffId)->delete();
        Holiday::query()->where('staff_id', $staffId)->delete();
        SpecialDay::query()->where('staff_id', $staffId)->delete();
        Timesheet::query()->where('staff_id', $staffId)->delete();
        Data::query()->where('table_name', 'staff')->where('row_id', $staffId)->delete();
    }
