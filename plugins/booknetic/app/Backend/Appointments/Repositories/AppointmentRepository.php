...
class AppointmentRepository
{
    ...
    public function getAppointments(array $filters, array $orderBy, string $search, int $skip, int $limit = 12): array
    {
        ...
        $appointmentsData = $appointments->fetchAll();
    ...
    public function searchServices(string $search = '', int $limit = 100): array
    {
        $query = Service::query()
            ->select([
                Service::getField('id'),
        ...
        return $query->fetchAllAsArray();
    }

    public function searchStaff(string $search = '', int $limit = 100): array
    {
        $query = Staff::query()
            ->where('is_active', 1)
            ->orderBy(Staff::getField('id'))
            ->limit($limit);
    ...
    public function searchCustomers(string $search = '', int $limit = 100): array
    {
        $query = Customer::my()
            ->select(['id', 'first_name', 'last_name'])
            ->orderBy(Customer::getField('id'))
            ->limit($limit);

        if ($search !== '') {
            $query->like("CONCAT(`first_name`, ' ', `last_name`)", $search);
        }

        return $query->fetchAllAsArray();
    }

    public function searchLocations(string $search = '', int $limit = 100): array
    {
        $query = Location::my()
            ->where('is_active', 1)
            ->orderBy(Location::getField('id'))
            ->limit($limit);
