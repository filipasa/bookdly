...
class AppointmentRequestData extends \stdClass
{
    ...
    public function __get(string $name)
    {
        ...
        switch ($name) {
            case 'appointmentInf':
                $this->appointmentInf = Appointment::query()->get($this->appointmentId);
                break;
            case 'serviceInf':
                $this->serviceInf = Service::query()->withTranslations()->get($this->serviceId);
                break;
            case 'locationInf':
                $this->locationInf = Location::query()->withTranslations()->get($this->locationId);
                break;
            case 'serviceStaffInf':
                $serviceStaffInf = ServiceStaff::query()
                    ->withTranslations()
    ...
    public function initDefaultProperties()
    {
        ...
        $this->locationCategoryId	        =   $this->getData('location_category', 0, 'int');
    ...
    public function getServiceExtras()
    {
        if (is_null($this->serviceExtras)) {
            ...
            foreach ($serviceExtras as $extraDetails) {
                ...
                $extraObj = ServiceExtra::query()->where('id', $extraDetails['extra']);
    ...
    public function handleCustomerInfo()
    {
        ...
        if ($customerId > 0) {
            $validateCustomerId = Customer::query()->get($customerId);
            ...
            if (! $validateCustomerId) {
                ...
                $this->customerData['category_id'] = $validateCustomerId->category_id;
            }
        }

        if ($this->calledFromBackend) {
            $this->customerId = $customerId;
            $this->customerDataObj->category_id = $this->customerData['category_id'] ?? 0;
        } else {
            $this->customerDataObj = new CustomerData();
            ...
            $this->customerDataObj->category_id = $this->customerData['category_id'] ?? '';
    ...
    public function validateBasicData()
    {
        ...
        $isStaffAllowed = Staff::query()->where('id', $this->staffId)->whereFindInSet('locations', $this->locationInf->id)->fetch();
    ...
    {
        do_action('bkntc_validate_customer_booking', $this);
        ...
        if ($this->calledFromBackend) {
            ...
            $checkCustomerExists = Customer::query()->get($this->customerId);
    ...
    public function hasDuplicateBooking(): bool
    {
        ...
        $count = Appointment::query()
            ->where('starts_at', $timeslots[ 0 ]->getTimestamp())
    ...
    private function validateServiceExtras($min, $max): void
    {
        ...
        if ($min === $max) {
            throw new \RuntimeException(bkntc__("Error! You must select %s service extras. Selected: %s.", [$max, $count]));
        }

        throw new \RuntimeException(bkntc__("Error! Please select between %s and %s service extra(s). Selected: %s.", [$min, $max, $count]));
    }
}
