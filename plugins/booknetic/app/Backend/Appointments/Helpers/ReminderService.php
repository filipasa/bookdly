...
final class ReminderService
{
    ...
    private function triggerTenantNotifiedWorkflow(Collection $workflow, int $offset): void
    {
        ...
        foreach ($tenants as $tenant) {
            ...
            if (!in_array($workflow->id, $triggeredWorkflowIDs)) {
                do_action('bkntcsaas_tenant_notified', $tenant->id, $workflow->id);
    ...
    private function triggerAppointmentWorkflows(Collection $workflow, WorkflowFilterData $filter, array $actions): void
    {
        ...
        if (count($filter->getLocationCategories()) > 0) {
            $appointmentQuery
                ->leftJoin('location', ['category_id'])
                ->where(Location::getField('category_id'), 'IN', $filter->getLocationCategories());
        }
        ...
        try {
            if (!empty($workflow['data'])) {
                ...
                $filter->setOffset($offset)
                    ...
                    ->setLocationCategories($data['location_categories'] ?? []);
            }
        } catch (Exception $e) {
        }
