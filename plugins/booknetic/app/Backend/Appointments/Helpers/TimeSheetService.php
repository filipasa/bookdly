...
class TimeSheetService extends ServiceDefaults
{
    ...
    {
        if (!$this->getInsideWorkingHours() && $this->canBookAnyTime()) {
    ...
    public function getDay(int $dayOfWeek): TimeSheetObject
    {
     ...
     * @return TimeSheetObject[]
     */
    public function getSpecialTimesheet($date): array
    {
    ...
    {
        $timePriority = Helper::getOption('time_priority', 'staff');

        if ($timePriority === 'merge') {
            return $this->getMergedTimeSheet();
        }

        if ($timePriority == 'service') {
            $firstOrder = 'service_id DESC';
            $secondOrder = 'staff_id DESC';
        } else {
            $firstOrder = 'staff_id DESC';
            $secondOrder = 'service_id DESC';
        }
        ...
        $timesheet = Timesheet::where(function ($qb) {
            ...
            ->orderBy([$firstOrder,$secondOrder])->fetch();

        return new WeeklyTimeSheetObject(json_decode($timesheet['timesheet'], true));
    }

    private function getMergedTimeSheet()
    {
        $timesheets = Timesheet::where(function ($qb) {
            $qb->where('service_id', 'is', null)
                ->where('staff_id', 'is', null);
        })->orWhere('service_id', $this->serviceId)
            ->orWhere('staff_id', $this->staffId)
        ...
        foreach ($timesheets as $ts) {
            if (!empty($ts['staff_id'])) {
                $staffTs = $ts;
            } elseif (!empty($ts['service_id'])) {
                $serviceTs = $ts;
            } else {
                $globalTs = $ts;
            }
        ...
        $staffSheet = new WeeklyTimeSheetObject(
            json_decode(($staffTs ?? $globalTs)['timesheet'], true)
        );

        $serviceSheet = new WeeklyTimeSheetObject(
            json_decode(($serviceTs ?? $globalTs)['timesheet'], true)
        );

        return static::intersectTimesheets($staffSheet, $serviceSheet);
    }

    /**
     * Calculates the intersection of two weekly timesheets.
     * Per day: start = max(start1, start2), end = min(end1, end2).
     * Breaks: union of both timesheets' breaks.
     * No overlap (start >= end) results in day_off.
     */
    public static function intersectTimesheets(
        WeeklyTimeSheetObject $ts1,
        WeeklyTimeSheetObject $ts2
    ): WeeklyTimeSheetObject {
        $result = [];

        for ($day = 0; $day < 7; $day++) {
            $d1 = $ts1->getDay($day)->toArr();
            $d2 = $ts2->getDay($day)->toArr();

            if ($d1['day_off'] || $d2['day_off']) {
                $result[] = ['day_off' => 1, 'start' => '', 'end' => '', 'breaks' => []];
                continue;
            }

            $start = Date::epoch($d1['start']) > Date::epoch($d2['start']) ? $d1['start'] : $d2['start'];
            $end = Date::epoch($d1['end']) < Date::epoch($d2['end']) ? $d1['end'] : $d2['end'];

            if (Date::epoch($start) >= Date::epoch($end)) {
                $result[] = ['day_off' => 1, 'start' => '', 'end' => '', 'breaks' => []];
                continue;
            }

            $breaks = array_merge($d1['breaks'], $d2['breaks']);

            $result[] = ['day_off' => 0, 'start' => $start, 'end' => $end, 'breaks' => $breaks];
        }

        return new WeeklyTimeSheetObject($result);
    }
    ...
    private function canBookAnyTime()
    {
        if ($this->calledFromBackEnd) {
            ...
            if (! Permission::isAdministrator() && Capabilities::userCan('appointment_book_outside_working_hours')) {
                return true;
            }
        }
