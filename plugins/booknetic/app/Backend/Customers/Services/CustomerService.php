...
class CustomerService
{
     ...
     */
    public function deleteAll(array $ids, int $deleteWpUser): void
    {
        if (empty($ids)) {
            throw new InvalidCustomerDataException(
                bkntc__('Please select at least one user!')
            );
        }

        $deleteWpUser = $this->canDeleteWpUser($deleteWpUser);

        $checkAppointments = $this->customerAppointmentRepository
                ->getAppointmentCount($ids);
        ...
        foreach ($ids as $id) {
            $this->deleteCustomerById((int)$id, $deleteWpUser);
        }
    }
     ...
     * @throws CustomerHasAppointmentException
     */
    public function delete(int $id, int $deleteWpUser): void
    {
        if ($id <= 0) {
            throw new InvalidCustomerDataException(
                bkntc__('Please select a valid user!')
            );
        }

        $deleteWpUser = $this->canDeleteWpUser($deleteWpUser);

        $appointmentCount = $this->customerAppointmentRepository
                ->getAppointmentCount([$id]);

        if ($appointmentCount > 0) {
            throw new CustomerHasAppointmentException();
        }

        $this->deleteCustomerById($id, $deleteWpUser);
    }

    /**
     * @param   int   $id
     * @param   bool  $deleteWpUser
     *
     * @return void
     */
    private function deleteCustomerById(int $id, bool $deleteWpUser): void
    {
        ...
        if ($customerInf->user_id > 0) {
            ...
            $customerCountForWPUser = $this->repository
                    ->getCustomerCountByWpUserId($customerInf->user_id);

            if (
                $userData &&
                $customerCountForWPUser === 1 &&
                in_array('booknetic_customer', $userData->roles, true)
            ) {
    ...
    /**
     * @param   int  $deleteWpUser
     *
     * @return bool
     */
    private function canDeleteWpUser(int $deleteWpUser): bool
    {
        return $deleteWpUser === 1 &&
                (Permission::isAdministrator() ||
                        Capabilities::userCan('customers_delete_wordpress_account'));
    }
    ...
    public function importCustomer(array $file): void
    {
        ...
        $allCustomerCategories = $this->customerCategoryRepository->getAll();
        $categoryToId          = [];

        if (!empty($allCustomerCategories)) {
            $categoryToId = array_merge(
                ...array_map(fn ($category) => [$category->name => $category->id], $allCustomerCategories)
            );
        }
        ...
        foreach (explode(',', $fields) as $fieldName) {
            if (in_array(
                $fieldName,
                ['category', 'first_name', 'last_name', 'email', 'phone_number', 'gender', 'birthdate', 'notes']
            )) {
                $fields1[] = $fieldName;
            }
        }
        ...
        foreach ($csvArray as $rows) {
            ...
            if (isset($insertData['birthdate'])) {
                $insertData['birthdate'] = Date::isValid(
                    $insertData['birthdate']
                ) ? $insertData['birthdate'] : str_replace('/', '-', $insertData['birthdate']);
                $insertData['birthdate'] = empty($insertData['birthdate']) ? null : Date::dateSQL(
                    $insertData['birthdate']
                );
            }
            ...
            if (isset($insertData['category']) && $insertData['category'] != '-') {
                $categoryName = $insertData['category'];
                if (!empty($categoryToId[$categoryName])) {
                    $insertData['category_id'] = $categoryToId[$categoryName];
                }

                unset($insertData['category']);
            }
