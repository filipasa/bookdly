...
class CustomerModule implements IModule
{
    ...
    public static function registerRoutes(): void
    {
        ...
        if (Capabilities::tenantCan('customer_categories')) {
    ...
    {
        $rest = new RestGroup('customers');
        $controller = Container::get(CustomerRestController::class);
        $rest->get('', [$controller, 'getAll']);
        $rest->get('(?P<id>\d+)', [$controller, 'get']);
        $rest->post('', [$controller, 'create']);
        $rest->put('(?P<id>\d+)', [$controller, 'update']);
        $rest->delete('(?P<id>\d+)', [$controller, 'delete']);

        RestRoute::get('/customer/info/(?P<id>\d+)', [Container::get(CustomerRestController::class), 'get']);
        RestRoute::post('/customer/delete', [Container::get(CustomerRestController::class), 'deleteAll']);
    ...
    public static function registerMenu(): void
    {
        ...
        if (Capabilities::tenantCan('customer_categories') && Capabilities::userCan('customer_categories')) {
            MenuUI::get('customers')
