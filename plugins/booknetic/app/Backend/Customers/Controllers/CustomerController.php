    ...
    public function index(): void
    {
        try {
            ...
            $category = CustomerCategory::query()
                ...
                ->limit(1);
            $totalVisitsSubQuery = Appointment::query()
                ->where('customer_id', '=', DB::field('id', 'customers'))
                ->select('COUNT(*)', true);
            ...
            $dataTable = new DataTableUI(
                ...
                ->selectSubQuery($totalVisitsSubQuery, 'total_visits')
            );
            ...
            $dataTable->addColumns(bkntc__('TOTAL VISITS'), 'total_visits');
            ...
            wp_die($e->getMessage() . '<pre>' . $e->getTraceAsString() . '</pre>');
        }
    }
