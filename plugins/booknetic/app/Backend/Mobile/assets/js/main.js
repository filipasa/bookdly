(function ($) {
    ...
    function loadView(viewName) {
        booknetic.ajax('mobile_app_view.' + viewName, {}, function (result) {
            $('.mobile-app-view-area').html(result.html);

            $('.mobile-app-navigation li').removeClass('active');
            $('.mobile-app-navigation a[data-view="' + viewName + '"]').closest('li').addClass('active');
        });
    }

    $document.ready(function () {

        $document.on('click', '.mobile-app-navigation a[data-view]', function (e) {
            e.preventDefault();
        ...
        if (typeof mobileAppInitialView !== 'undefined') {
            loadView(mobileAppInitialView);
        }
