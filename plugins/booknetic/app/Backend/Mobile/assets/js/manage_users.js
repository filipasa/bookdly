    ...
    $document.ready(function () {
        ...
        $document.on('click', '.assign-user-confirm', function () {
            ...
            booknetic.ajax('mobile_app_seat.assign', formData, (response) => {
                ...
                updateCredentialsQrCode(response.data.username, response.data.password);
        ...
        function regeneratePasswordAjax(username, seatId) {
            ...
            booknetic.ajax('mobile_app_seat.regenerate_password', data, (result) => {
                ...
                updateCredentialsQrCode(result.username, result.app_password);

                booknetic.newModal.open('.seat-credentials-modal');
            });
        }

        function updateCredentialsQrCode(username, password) {
            const $modal = $('.seat-credentials-modal').last();
            const websiteUrl = $modal.find('.website-url').first().text().trim();
            ...
            const qrText = 'booknetic://qr-login?data=' + encodeURIComponent(JSON.stringify(credentials));
            const qrUrl = 'https://quickchart.io/qr?text=' + encodeURIComponent(qrText);

            const $qrImage = $modal.find('.credentials-qr-code');
            const $qrLoading = $modal.find('.qr-loading');
