...
class SeatAjaxController extends Controller
{
    ...
    public function manage_seats_modal(): array
    {
        ...
        if ($response->isProduct()) {
            return $this->response(false, bkntc__('Your subscription is managed through your Product, please go to the billing in settings to update it'));
        }
    ...
    public function update_seat_preview()
    {
        ...
        $result = $this->subscriptionService->getPreview($seatCount);

        return $this->response(true, ['result' => $result]);
    }
