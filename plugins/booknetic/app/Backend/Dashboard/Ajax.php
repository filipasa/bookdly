...
class Ajax extends Controller
{
    ...
    public function get_upcoming_appointments()
    {
        ...
        $showAll = (bool) Post::int('show_all');
        $offset  = Post::int('offset');
        $type    = Post::string('type', 'today');
        $start   = Post::string('start');
        $end     = Post::string('end');

        $result = $this->dashboardService->getUpcomingAppointments($showAll, $offset, $type, $start, $end);

        return $this->response(true, $result->jsonSerialize());
    }
    ...
    public function get_graph_data()
    {
        ...
        if ($year === 'last_year') {
            $startDate = Date::dateSQL(Date::epoch('today', '-1 year'));
            $endDate = Date::dateSQL();
        } else {
            $startDate = Date::dateSQL(Date::epoch("$year-01-01"));
            $endDate = Date::dateSQL(Date::epoch($startDate, '+1 year -1 day'));
        }
