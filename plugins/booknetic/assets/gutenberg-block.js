(function ( element, blocks, editor, components )
{
	...
	var location_categs = [ {value: 0, label: '-----'} ];
	location_categs = location_categs.concat(BookneticData.location_categs);
	...
	registerBlockType( 'booknetic/booking', {
		...
		edit: function( props )
		{
			...
			function onChangeLocationCateg( categId )
			{
				props.setAttributes( { location_categ: parseInt( categId ) } );
			}
			...
			function shortCode( props )
			{
				...
				if( props.attributes.location_categ > 0 )
				{
					attrs.push( 'location_category=' + props.attributes.location_categ )
				}
				...
				el(
					...
					el(
						...
						el(
							SelectControl,
							{
								label: 'Location category filter',
								value: props.attributes.location_categ,
								options: location_categs,
								onChange: onChangeLocationCateg
							}
						),
						el(
