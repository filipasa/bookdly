...
class Config
{
    ...
    public static function registerCoreWorkflowEvents()
    {
        ...
        add_action('bkntcsaas_tenant_notified', function ($tenantId, $workflowId) {
            $prev_tenantId = PermissionRegular::tenantId();

            PermissionRegular::setTenantId(null);

            self::$workflowEventsManager->trigger('tenant_notified', [ 'tenant_id' => $tenantId ], false, false, null, $workflowId);
    ...
    {
        Container::addBulk([
            \BookneticSaaS\Backend\Base\Ajax::class
        ]);

        SaaSRoute::post('base', Container::get(\BookneticSaaS\Backend\Base\Ajax::class));
    ...
    public static function registerTenantActivities()
    {
        ...
        if (Route::getCurrentModule() !== 'billing' && PermissionRegular::tenantId() > 0) {
            $tenantInf = PermissionRegular::tenantInf();
            if ($tenantInf && Date::epoch(Date::dateSQL()) > Date::epoch($tenantInf->expires_in) && ! Tenant::haveEnoughBalanceToPay()) {
                if (wp_doing_ajax()) {
                    echo json_encode(\BookneticApp\Providers\Helpers\Helper::response(false, bkntcsaas__('Your plan has expired. Please upgrade your subscription.')));
                    exit;
                } else {
                    \BookneticApp\Providers\Helpers\Helper::redirect(Route::getURL('billing'));
                }
