...
class Ajax extends \BookneticApp\Providers\Core\Controller
{
    ...
    public function get_stripe_portal_url()
    {
        $tenantInf = Permission::tenantInf();
        $subscriptionId = $tenantInf->active_subscription;
        ...
        if (empty($subscriptionId)) {
            ...
            if ($latestCreditCardBilling && !empty($latestCreditCardBilling->agreement_id)) {
                $subscriptionId = $latestCreditCardBilling->agreement_id;
            }
        }

        if (empty($subscriptionId) || strpos($subscriptionId, 'sub_') !== 0) {
            return $this->response(false, bkntcsaas__('No active Stripe subscription found.'));
        }
        ...
        try {
            ...
            $session = \Stripe\BillingPortal\Session::create([
                'customer' => $customerId,
                'return_url' => \BookneticApp\Providers\Core\Route::getURL('billing')
            ]);

            return $this->response(true, [ 'url' => $session->url ]);
        } catch (\Exception $e) {
            return $this->response(false, $e->getMessage());
        }
    }
