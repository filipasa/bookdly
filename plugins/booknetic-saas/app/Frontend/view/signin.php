...
<div class="bookneticsaas_login">
	...
	<?php if (\BookneticApp\Providers\Helpers\Helper::getOption('google_login_enable', 'off') == 'on'): ?>
		...
		jQuery(document).ready(function($) {
			$('.bkntc-saas-google-login-btn').on('click', function() {
				var loginUrl = '<?php echo site_url() . "/?" . \BookneticApp\Providers\Helpers\Helper::getSlugName() . "_action=google_login"; ?>';
				var win = window.open(loginUrl, 'Google Login', 'width=500,height=600');
					...
					try {
						if (win.booknetic_login_status) {
							...
							if (status === 'success') {
								...
								alert('This Google account is not registered as a tenant. Redirecting to registration...');
								var signupUrl = '<?php 
									...
									echo $signup_page_id ? get_permalink($signup_page_id) : site_url("/sign-up/"); 
								?>';
								window.location.href = signupUrl + '?email=' + encodeURIComponent(data.email || '') + '&name=' + encodeURIComponent(((data.first_name || '') + ' ' + (data.last_name || '')).trim());
							}
						}
					} catch(e) {}
				}, 500);
