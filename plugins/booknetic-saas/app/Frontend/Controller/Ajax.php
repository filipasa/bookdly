...
class Ajax extends FrontendAjax
{
    ...
    public function signup()
    {
        ...
        $password		=	Helper::_post('password', '', 'string');
        $requested_plan_id = Helper::_post('plan_id', 0, 'int');
        $billing_cycle = Helper::_post('billing_cycle', 'monthly', 'string', ['monthly', 'annually']);
        ...
        // Use requested plan if valid, otherwise fall back to the default plan
        $selectedPlan = null;
        if ($requested_plan_id > 0) {
            $selectedPlan = Plan::where('id', $requested_plan_id)->fetch();
        }
        if (!$selectedPlan) {
            $selectedPlan = Plan::where('is_default', 1)->fetch();
        }
        ...
        $price = $billing_cycle === 'monthly' ? $selectedPlan->monthly_price : $selectedPlan->annually_price;

        error_log("Signup Debug: price=" . $price . ", stripe_enable=" . Helper::getOption('stripe_enable') . ", billing_cycle=" . $billing_cycle . ", plan_id=" . $selectedPlan->id);
        ...
        if ($price > 0) {
            ...
            $checkout = new \BookneticSaaS\Integrations\PaymentGateways\Stripe();
            $checkout->setAmount($price, $price, Helper::getOption('currency', 'USD', false));
            $checkout->setId($billingId);
            $checkout->setCycle($billing_cycle);
            $checkout->setPlan($selectedPlan);
            $checkout->setEmail($email);
            $checkout->setTrialDays($trialPeriod);
            
            $success_url = site_url() . '/sign-up/?payment_status=success&email=' . urlencode($email) . '&bkntc_session_id={CHECKOUT_SESSION_ID}';
            $cancel_url = site_url() . '/sign-up/?payment_status=cancel';
            
            $checkout->setSuccessURL($success_url);
            $checkout->setCancelURL($cancel_url);

            $checkoutResult = $checkout->createRecurringPayment();

            if ($checkoutResult['status']) {
                return $this->response(true, [ 'redirect_url' => $checkoutResult['url'] ]);
            } else {
                // Cleanup
                wp_delete_user($tenantUserId);
                Tenant::noTenant()->where('id', $tenantId)->delete();
                TenantBilling::noTenant()->where('id', $billingId)->delete();

                return $this->response(false, $checkoutResult['error']);
            }
        } else {
            // Free plan - normal confirm & verify flow
