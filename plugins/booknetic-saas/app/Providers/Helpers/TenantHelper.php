...
class TenantHelper
{
    ...
    */
    public static function delete($tenant)
    {
        ...
        self::cancelActiveSubscription($tenant);
    ...
    private static function cancelActiveSubscription($tenant): void
    {
        if (empty($tenant->active_subscription)) {
            return;
        }

        $billingInf = TenantBilling::noTenant()
            ->where('agreement_id', $tenant->active_subscription)
        ...
        try {
            switch ($billingInf->payment_method) {
                case 'paypal':
                    (new Paypal())->cancelSubscription($billingInf->agreement_id);
                    break;
                case 'credit_card':
                    (new Stripe())->cancelSubscription($billingInf->agreement_id);
                    break;
            }
        } catch (\Exception $e) {
            // Subscription cancel uğursuz olsa belə, tenant silmə prosesi davam etməlidir
        }
    }
