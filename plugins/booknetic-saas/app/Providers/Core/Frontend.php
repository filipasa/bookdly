...
class Frontend
{
    ...
    private static function stripeConfirm()
    {
        ...
        if ($result[ 'status' ] === true) {
            Tenant::billingStatusUpdate($result[ 'billing_id' ], $result[ 'subscription' ], $result['expire_date'] ?? null, $result['invoice_id']);
    ...
    private static function addShortcodes()
    {
        ...
        add_shortcode('booknetic-saas-signup', function ($atts) {
            ...
            // Verify Stripe checkout callback during signup
            $paymentStatus = Helper::_get('payment_status', '', 'string');
            $sessionId = Helper::_get('bkntc_session_id', '', 'string');

            if ($paymentStatus === 'success' && !empty($sessionId)) {
                $payment = new Stripe();
                $result = $payment->checkSession($sessionId);
                ...
                if ($result['status'] === true) {
                    ...
                    if ($billingInfo && $billingInfo->status !== 'paid') {
                        Tenant::billingStatusUpdate($billingId, $result['subscription'], $result['expire_date'] ?? null, $result['invoice_id']);
                        
                        // Fire confirmation email now that the subscription card has been authorized
                        do_action('bkntcsaas_tenant_sign_up_confirm', $billingInfo->tenant_id);
                    }
                }
            }
    ...
    public static function tenantBookingPage(array $posts, WP_Query $query): array
    {
        ...
        if (\BookneticSaaS\Providers\Helpers\Date::epoch(\BookneticSaaS\Providers\Helpers\Date::dateSQL()) > \BookneticSaaS\Providers\Helpers\Date::epoch($tenant->expires_in) && !Tenant::haveEnoughBalanceToPay()) {
            wp_die(
                '<div style="text-align: center; margin-top: 50px; font-family: sans-serif; color: #333;">' .
                '<h2>' . bkntcsaas__('Booking Page Suspended') . '</h2>' .
                '<p>' . bkntcsaas__('This booking page is temporarily unavailable because the subscription has expired.') . '</p>' .
                '</div>',
                bkntcsaas__('Booking Page Suspended'),
                ['response' => 403]
            );
        }
